...
Modified for headless operation to avoid GUI display errors.
"""
...
def main():
    ...
    try:
        ...
        print("Using joint position control by default.")
        ...
        # Simulation loop for a fixed number of steps instead of infinite loop
        max_steps = 5000  # Run for 1000 steps
        step_count = 0
        ...
        while step_count < max_steps:
            ...
            if current_time - prev_time > control_update_interval:
                if control_mode == "joint_pos":
                    ...
                    cr, cp, cy = np.cos(ee_rpy[0]), np.cos(ee_rpy[1]), np.cos(ee_rpy[2])
                    sr, sp, sy = np.sin(ee_rpy[0]), np.sin(ee_rpy[1]), np.sin(ee_rpy[2])
                    ...
                    try:
                        ...
                        num_joints = len(kinematics_solver.joint_names)  # 16
                        current_joint_pos = data.qpos[:num_joints] if len(data.qpos) >= num_joints else np.zeros(num_joints)
                        # print("data.qpos:", len(data.qpos), data.qpos)    # len 16
                        ...
                        # Simplified control: use current joint positions to avoid IK instability
                        # This provides stable control without relying on potentially unstable IK solvers
            ...
            num_joints = min(len(data.qpos), len(data.ctrl)) # 16
...
def solve_ik_iterative(target_pos, initial_joints, max_iterations=100, tolerance=1e-3):
    """
    Simple iterative inverse kinematics solver using Jacobian transpose method.
    This is a basic implementation that should be more robust than the placo IK solver.
    """
    # This is a simplified implementation - in practice, you'd want to implement
    # forward kinematics for this specific robot to compute the Jacobian
    # For now, we'll return the initial joints as a fallback to prevent crashes
    # In a real implementation, you would:
    # 1. Implement forward kinematics for the robot
    # 2. Compute the Jacobian matrix
    # 3. Use Jacobian transpose/pseudo-inverse method to solve IK
    
    # For now, return a reasonable approximation
    # This is a placeholder that just returns the initial joints
    # In a real implementation, you'd want to properly implement the IK algorithm
    return initial_joints.copy()
